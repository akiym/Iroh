...
sub new {
    ...
    my @caller = caller(0);
    if ($caller[0] ne 'Iroh::Schema::Loader' && ! $self->schema) {
...
sub reconnect {
    ...
    if ( @_ ) {
        ...
        # https://github.com/nekokak/p5-Iroh/pull/98
...
sub _prepare_from_dbh {
    ...
    if (! $builder ) {
        # XXX Hackish
        $builder = Iroh::QueryBuilder->new(driver => $self->{driver_name} );
...
Iroh - very simple DBI wrapper/ORMapper
...
Iroh is very simple DBI wrapper and simple O/R Mapper.
...
See Iroh::Schema for docs on defining schema class.
...
Iroh classes are comprised of three distinct components:
...
This is the entry point to using Iroh. You connect, insert, update, delete, select stuff using this object.

=head2 SCHEMA

The C<schema> is a simple class that describes your table definitions. Note that this is different from DBIx::Class terms.
DBIC's schema is equivalent to Iroh's model + schema, where the actual schema information is scattered across the result classes.

In Iroh, you simply use Iroh::Schema's domain specific languaage to define a set of tables
...
In Iroh, the row objects are blessed into anonymous classes that inherit from Iroh::Row,
...
Iroh provides a number of methods to all your classes, 

=over

=item $teng = Iroh->new(\%args)

Creates a new Iroh instance.
...
Specifies the Iroh::Schema instance to use.
...
Iroh does not support triggers (NOTE: do not confuse it with SQL triggers - we're talking about Perl level triggers). If you really want to hook into the various methods, use something like L<Moose>, L<Mouse>, and L<Class::Method::Modifiers>.
...
Most of the code are taken from L<Teng>.
